...
# Windows 10 homeassistant power management
...
hower solves issues I ran into following other posts.*

I wanted to be able to, lazily from my bed, turn on/off my desktop PC. Having recent set up
home assistant, the logical aproach was to integrate it into there. I already had my computer 
...
Before you even start trying to use the homeassistant WoL integration, you first have to ensure
...
which could be confusing, as there's no mention of WoL there, however as the network cardsa are 
connected over PCI-E, this is the setting to enable on those boards.

Once you've got WoL working, adding the
[homeassistant WoL integration](https://www.home-assistant.io/integrations/wake_on_lan/) 
...
addresses sorta kinda censored.
...
Once you've got that set up, you may run into an issue where the UI in homeassistant always thinks
...
requests. To fix this such that homeassistant can see that the PC is on, open the
...
![firewall_rule.png](/homeassistant_pc_power/firewall_rule.png)
...
# 2 Turnng off the PC - SSH
While there is an RPC Shutdown plugin for homeassistant, I didn't have much luck with it. It seems
...
great idea to give your homeassistant instance unlimited access to your main account. Again the commands
...
then, to allow your homeassistant instance to connect to this user without a password, we need to set up
key login. SSH into your homeassistant instance, and generate a key. In order to ensure the key is persisted,
put it in a persistant folder like `/config` or `/ssl`.

logged in to your homeassistant instance over SSH, run the following command to generate a new key:
```bash
...
Next you'll have to allow this key to connect to the homeassistant user on your W10 PC. Run:
```bash
...
![authorized_keys Properties](/homeassistant_pc_power/authorized_keys_permissions1.png)

1) Disable inherritance at the bottom and convert to explicit permissions
1) Change the owner to homeassistant at the top, click change and enter `homeassistant` in the box
...
![authorized_keys Properties](/homeassistant_pc_power/authorized_keys_permissions2.png)

Once this is set up, we're almost there, however the homeassistant account needs to have permissions to force a shutdown. If you have windows 10 *pro* or similar, this can be done via the Group Policy Editor easily.
...
# 3 Adding the turn off part to the homeassistant switch
Now that all the pieces are in place, we just have to tell homeassistant how to nicely ask the computer to 
turn off.
...
```yaml
shell_command:
  desktop_off: 'ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=1 -o ServerAliveCountMax=1 -i /ssl/.ssh/id_ed25519 homeassistant@10.0.0.5 "shutdown /s /t 60 /c \"Homeassistant is shutting down the PC in 60 seconds. Run shutdown /a in a cmd if you want to abort it\""'
```
